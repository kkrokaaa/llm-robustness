...
import numpy as np
'''
# if not using <lib.attack.Prompt>
class PromptGeneratorForTruthfulQA:
    def __init__(self, target_model):
        self.target_model = target_model
    def generate_prompt(self, data, max_new_len=100):
        max_new_tokens = max( len(self.target_model.tokenizer(data['incorrect_answers']).input_ids) +2, max_new_len )
        ...
        #data['generated_prompt'] = attacks.Prompt(full_prompt, perturbable_prompt, max_new_tokens)
        data['full_prompt'] = full_prompt
        data['perturbable_prompt'] = perturbable_prompt
        data['max_new_tokens'] = max_new_tokens
        return data                     # ft fed in .map should return dict     /   values should not be Prompt?
'''
