...
def load_data_datasetwise(tokenizer: transformers.PreTrainedTokenizer,
    ...
    if cache:
        if cache_dir is None:
            ...
            print(f'cache directory not given to the argument "cache_local"; automatically set as {cache_dir}')
        caching_path = str( Path(cache_dir) / f"mmlu_{tokenizer.__class__.__name__}_exmp{num_example}_merge{merge_split}_conv{conv_generation}" )
    ...
    for task in tqdm(task_list):                                                                                  # ex) task : anatomy
        data_set = load_dataset(data_source, task)
        data_set.save_to_disk(save_dir + '/' + task)
        
        for task_name, data_set in mmlu_formatter_datasetwise(tokenizer,                                    # ex) task_name : mmlu__anatomy__test
                                                              dataset_path = save_dir + '/' + task, 
                                                              num_example = num_example,
                                                              cache = cache,
                                                              merge_split = merge_split,
                                                              conv_generation = conv_generation,
...
def mmlu_formatter_datasetwise(
    tokenizer: transformers.PreTrainedTokenizer,
    dataset_path: str = "./data/mmlu",
    num_example: int = 5,
    cache: bool = True,
    merge_split: bool = False,
    conv_generation: bool = True,
    ...
    dd = datasets.load_from_disk(dataset_path)
    
    if merge_split:
        ds = datasets.concatenate_datasets([dd[split] for split in dd.keys()])
        ds_key = Path(dataset_path).stem                                                        # task name (name of the subdir folder)
        ds_and_key = [(ds, ds_key)]                                                             # [(dataset, task name)]
    else:
        ds_and_key = [(ds, Path(f).stem + "__" + split) for split, ds in dd.items()]            # [(dataset, task name__train), (dataset, task name__validation), (dataset, task name__test)]
    ...
    for ds, ds_key in ds_and_key:
        ...
        return_dataset = datasets.Dataset.from_pandas(pd.DataFrame(return_dataset))
        
        if cache:
            return_dataset.save_to_disk(caching_path + '/mmlu__' + ds_key)

        yield 'mmlu__' + ds_key, return_dataset